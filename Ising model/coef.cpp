...
float T = 2.25;                        // Curie temperature
...
long unsigned int mcs = 1000000;        // number of Monte Carlo steps
...
int main()
{
    ...
    float T1 = T + change, T2 = T + change - 0.005;
    float C1, C2, M1, M2, X1, X2;

    // T1 monte-carlo cycle
    T = T1;
    transient_results();
    ...
    M1 = Mabs_avg;
    X1 = (Msq_avg-(Mabs_avg * Mabs_avg * n)) / (T);
    C1 = (Esq_avg-(E_avg * E_avg * n)) / (T * T);
    std::cout << M1 << std::endl; 
    std::cout << X1 << std::endl; 
    std::cout << C1 << std::endl; 


    // T2 monte-carlo cycle
    T = T2;
    transient_results();
    M = total_magnetization();
    Mabs = abs(total_magnetization());
    ...
    //         "\t" << 1-((Mq_avg) / (3 * Msq_avg)) << std::endl;           // cumulant (U_L)
    M2 = Mabs_avg;
    X2 = (Msq_avg-(Mabs_avg * Mabs_avg * n)) / (T);
    C2 = (Esq_avg-(E_avg * E_avg * n)) / (T * T);
    std::cout << M2 << std::endl; 
    std::cout << X2 << std::endl; 
    std::cout << C2 << std::endl; 


    std::cout << log(M2 / M1)/log(abs(T2 - 2.25)/abs(T1 - 2.25)) << '\n';
    std::cout << log(X2 / X1)/log(abs(T2 - 2.25)/abs(T1 - 2.25)) << '\n';
    std::cout << log(C2 / C1)/log(abs(T2 - 2.25)/abs(T1 - 2.25)) << '\n';
